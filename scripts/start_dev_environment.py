...
def setup_dev_environment():
    ...
    print("\n🔐 Test du système d'accès rapide...")
    try:
        from lib.auth.quick_access import get_quick_access_system
        
        system = get_quick_access_system()
        status = system.get_access_status()
        
        print("  ✅ Système d'accès rapide chargé")
        print(f"  📊 Statut: {status['quick_access_enabled']}")
        print(f"  🔑 Code secret: {status['secret_code_length']} caractères")
