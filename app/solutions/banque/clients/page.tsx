...
export default function ClientsList() {
  ...
  async function fetchClients() {
    setLoading(true);
    const { data } = await supabase.from("bank_clients").select();
    setClients(data || []);
    setLoading(false);
  }
  ...
  async function handleDelete(id: string) {
    ...
    await supabase.from("bank_clients").delete().eq("id", id);
    fetchClients();
  }
            ...
            <Card key={client.id}>
              ...
              <CardContent>
                ...
                <div className="flex gap-2 mt-2">
                  <Link href={`/solutions/banque/clients/${client.id}`} className="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-background hover:bg-accent hover:text-accent-foreground h-9 rounded-md px-3">Éditer</Link>
