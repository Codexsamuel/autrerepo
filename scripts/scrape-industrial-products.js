...
const { scrapeAllIndustrialMarkets, saveScrapedProducts, updateMainCatalog } = require('../lib/scraper/industrial-tech-scraper.ts');
...
async function main() {
  ...
  try {
    ...
    console.log(`- Average price: $${(products.reduce((sum, p) => sum + p.price, 0) / products.length).toFixed(2)}`);
