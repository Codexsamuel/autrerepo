...
interface AmazonProductsClientProps {
...
export default function AmazonProductsClient({
  ...
  // Fonction pour charger les produits Amazon
  const loadAmazonProducts = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/amazon/products?${params}`);
      
      if (!response.ok) {
        throw new Error('Erreur lors du chargement des produits Amazon');
      }
      ...
      console.error('Erreur lors du chargement des produits Amazon:', error);
      setProducts([]);
      setError('Erreur de connexion au serveur Amazon');
    } finally {
      setLoading(false);
    }
  };

  // Fonction pour charger les catégories Amazon
  const loadAmazonCategories = async () => {
    try {
      const response = await fetch('/api/amazon/products?action=categories');
      const data = await response.json();
      if (data.success) {
        setCategories(data.data || []);
      }
    } catch (error) {
      console.error('Erreur lors du chargement des catégories Amazon:', error);
    }
  };
  ...
  const handleAddToCart = (product: AmazonProduct) => {
    try {
      // Convertir AmazonProduct en format Product compatible
      const cartProduct = {
        id: product.id,
        name: product.title,
    ...
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-medium text-gray-900 mb-2">
          Erreur de chargement Amazon
        </h3>
        <p className="text-gray-600 mb-4">{error}</p>
        <Button onClick={loadAmazonProducts} variant="outline">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">
                Produits Amazon Premium
              </h1>
              <p className="mt-2 text-gray-600">
                Sélection de produits Amazon avec livraison au Cameroun
              </p>
            </div>
            ...
            <div className="mt-4 lg:mt-0 flex space-x-4">
              <div className="text-center">
                ...
                <div className="text-sm text-gray-600">Produits Amazon</div>
              </div>
              <div className="text-center">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredProducts.map((product) => (
            <Card key={`amazon-${product.id}`} className="overflow-hidden hover:shadow-lg transition-shadow">
